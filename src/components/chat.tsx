...
export default function Page({ agentId }: { agentId: UUID }) {
    ...
    useEffect(() => {
        ...
        // Setup periodic sync
        const intervalId = setInterval(syncMessages, 3000); // Sync every 3 seconds
        ...
        async function syncMessages() {
            try {
                ...
                queryClient.setQueryData(
                    ["messages", agentId],
                    (old: ContentWithUser[] = []) => {
                        // Use Set to deduplicate messages based on unique identifiers
                        const messageSet = new Set(old.map(msg => 
                            `${msg.id}`
                        ));
